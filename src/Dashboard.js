...
export class Dashboard extends React.Component{
    constructor(props){
        super(props);
        this.state = {showFriend: false,showExp:false,settleUp: false, bills: [{name: 'exp', totalAmount: 100, paidBy: {name: 'jon'}, isPaid: false, participants: [{name: 'jon'}, {name: 'khan'}]}]}
    }
...
componentDidMount(){
    ...
    if (this.state?.bills?.length === 0) {
        fetch('localHost:8000/bills').then(data => {
            this.setState(prev => ({ ...prev, bills: JSON.parse(data ?? '{}')}))
...
render(){
    ...
    <div >
        ...
        <div className ="flex" style={{ marginLeft: 'auto', marginRight: 'auto', width: 'fit-content'}}>
